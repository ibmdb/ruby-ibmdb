  ...
  def test_231_FieldTypeName
    assert_expect do
      ...
      for i in (0 .. IBM_DB::num_fields(result))
        puts "#{i}:#{IBM_DB::field_type(result,IBM_DB::field_name(result,i))}"
      end
      puts "\n-----"
      for i in (0 ... IBM_DB::num_fields(result2))
        puts "#{i}:#{IBM_DB::field_type(result2,IBM_DB::field_name(result2,i))}"
      end
      puts "\n-----"
      for i in (0 ... 3)
        puts "#{i}:#{IBM_DB::field_type(result3,IBM_DB::field_name(result3,i))}"
      end
      puts "\n-----"
