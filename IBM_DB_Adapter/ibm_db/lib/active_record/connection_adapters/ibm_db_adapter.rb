...
#       sql = "select * from sysibm.sqlcolumns where table_name = #{quote(table_name.upcase)}"
     ...
     def add_index(table_name, column_name, **options) #:nodoc:
      ...
      def table_exists?(table_name)
        puts_log "178"
        puts_log "IBM_DB table_exists = #{table_name}"
      ...
      def drop_table_indexes(index_list)
        puts_log "189 drop_table_indexes"
      ...
      def drop_column_indexes(index_list, column_name)
        puts_log "185 drop_column_indexes"
        index_list.each do |indexs|
...
#        puts "106"
      ...
      def change_column(table_name, column_name, type, options)
        @adapter.puts_log "109"
        @adapter.puts_log "#{table_name}, #{column_name}, #{type}"
      ...
      # DB2 Dataservers
      def set_binary_default(value)
        @adapter.puts_log "118"
        "BLOB('#{value}')"
      end

      # This method generates the blob value specified for DB2 Dataservers
      def set_binary_value
        @adapter.puts_log "119"
        "BLOB('?')"
      end
